...
export default function AppShell() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
      <div className="brand"><div className="brand-mark"><BookOpen size={21} /></div><div><strong>Senior .NET Cheatsheet</strong><span>Ôn nhanh để trả lời phỏng vấn</span></div><button className="mobile-close" onClick={() => setMobileOpen(false)} aria-label="Đóng menu"><X /></button></div>
      <nav className="sidebar-nav"><NavLink to="/" end className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><Home size={17} /> Lộ trình ôn</NavLink><NavLink to="/interview" className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><Sparkles size={17} /> Luyện phỏng vấn</NavLink><NavLink to="/review" className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><RotateCcw size={17} /> Hàng đợi ôn lại</NavLink>
