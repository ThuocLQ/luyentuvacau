import { BookOpen, Home, Menu, Moon, Search, Sparkles, Sun, X } from 'lucide-react'
import { NavLink, Outlet, useNavigate } from 'react-router-dom'
import { useEffect, useMemo, useState } from 'react'
import Fuse from 'fuse.js'
import { docs, sections } from '../data/docs'
...
export default function AppShell() {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`}>
      ...
      <nav className="sidebar-nav"><NavLink to="/" end className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><Home size={17} /> Interview map</NavLink><NavLink to="/interview" className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><Sparkles size={17} /> Interview mode</NavLink>
        {sections.filter(section => docs.some(doc => doc.section === section)).map(section => <div key={section}><div className="nav-label">{section}</div>{docs.filter(doc => doc.section === section).map(doc => doc.content ? <NavLink key={doc.slug} to={`/docs/${doc.slug}`} onClick={() => setMobileOpen(false)} className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}><span className="nav-dot" /><span>{doc.title.replace(/^\d+\. /, '')}</span><small>{doc.readingMinutes}p</small></NavLink> : <span className="nav-link nav-draft" key={doc.slug}><span className="nav-dot" /><span>{doc.title.replace(/^\d+\. /, '')}</span><small>Draft</small></span>)}</div>)}</nav>
      <div className="sidebar-footer"><span>21 focused cheatsheets · no filler</span></div>
    </aside>
