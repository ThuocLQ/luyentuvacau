...
const drafts: Draft[] = [
  {
    ...
    constraints: ['Concurrency limit là 40 request.', 'Không làm mất đơn chưa đồng bộ.'],
    ...
    recall: ['Async không làm đối tác có thêm capacity.', 'Concurrency limit phải đặt trước chỗ gọi downstream.'],
  ...
  },
  {
    ...
    facts: ['Đối tác nhận một operation ID cố định do app tạo.', 'Tạo vận đơn là side effect bên ngoài.', 'Đối tác có API query theo operation ID.'], constraints: ['Không tạo hai vận đơn cho một order.', 'Khi khách quay lại phải biết kết quả.'],
    prompt: 'App nên xử lý kết quả này thế nào?',
    options: [
      option('a', 'Persist trạng thái pending, query đối tác theo operation ID trước khi retry.', 'best', 'Đúng vì app chưa biết đối tác đã tạo vận đơn hay chưa. Query theo operation ID giúp xác minh kết quả thay vì đoán.'),
  ...
  {
    id: 'background-durability', version: 3, type: 'boundary', learningObjective: 'Hiểu durable handoff — persist job trước khi API báo nhận — và phân biệt provider nhận request với người dùng nhận email.', difficulty: 'Senior', relatedDoc: 'background-resilience', relatedSection: 'cach-quyet-dinh-tung-buoc',
    ...
    facts: ['Email hóa đơn không cần gửi xong trong request hiện tại.', 'Không được làm mất job gửi email.', 'Cùng một job có thể được giao lại cho worker.'], constraints: ['Không dùng scoped dependency của request sau khi request kết thúc.', 'Support cần biết email nào đang lỗi.'], prompt: 'Thiết kế nào đáng tin cậy nhất?',
    options: [
  ...
  },
  {
    ...
    options: [
      option('a', 'Lấy identity từ token và query order theo tenant cùng quyền của user.', 'best', 'Đúng vì server phải authorize trên chính resource đang được đọc. Token hợp lệ chỉ cho biết caller là ai, chưa cho biết họ được xem order nào.'),
    ...
    explanation: { decision: 'Server authorize trên đúng order đang được request.', mechanism: 'Authentication trả lời “ai gọi”; authorization trả lời “caller này được làm gì với resource này”.', tradeOff: 'Mọi order query đều phải filter theo tenant hoặc quyền của caller; đổi lại chặn được việc đoán hoặc sửa ID.', failureMode: 'Nếu chỉ ẩn trên UI hoặc tin tenant ID từ payload, user A có thể đọc order của B.', evidence: 'Integration test token của tenant A truy cập order của tenant B và theo dõi authorization denial.' }, recall: ['Authentication đúng chưa đủ để xem mọi resource.', 'Authorization phải được kiểm tra ở server trên đúng order.'], followUp: { changedConstraint: 'Support được xem nhiều tenant.', prompt: 'Bạn mở quyền thế nào?', expectedDirection: 'Tạo policy riêng, giới hạn rõ và lưu audit trail; không bỏ toàn bộ tenant filter.' }, tags: ['security', 'tenant']
  },
  {
  ...
  },
  {
    ...
    options: [
      option('a', 'Cho phép origin hợp lệ bằng CORS, đồng thời để server validate token và authorize trên resource.', 'best', 'Đúng vì hai việc giải quyết hai boundary khác nhau: CORS áp dụng cho browser, còn server quyết định caller có quyền đọc resource hay không.'),
    ...
    explanation: { decision: 'Dùng CORS cho browser boundary, nhưng authentication và authorization vẫn ở server.', mechanism: 'CORS cho browser biết origin nào được đọc response; token và policy ở server mới quyết định caller được đọc resource nào.', tradeOff: 'Phải cấu hình cả hai lớp, nhưng không nhầm vai trò của chúng.', failureMode: 'Chỉ dựa CORS sẽ để mobile hoặc HTTP client gọi API mà không bị authorize đúng cách.', evidence: 'Test cùng endpoint bằng browser, mobile-like client và token không có quyền xem resource.' }, recall: ['CORS không phải authentication hoặc authorization.', 'Resource authorization phải nằm ở server.'], followUp: { changedConstraint: 'Browser dùng cookie đăng nhập.', prompt: 'Cần xét thêm rủi ro nào?', expectedDirection: 'Xét CSRF và cookie policy, nhưng vẫn authorize trên resource ở server.' }, tags: ['security', 'cors']
  },
  {
    ...
    options: [
      option('a', 'Project đúng cột màn hình cần, sort ổn định theo CreatedAt rồi ID, và page trước khi materialize.', 'best', 'Đúng vì giảm số row và column cần đọc. ID làm tie-breaker để page không nhảy; sau đó xem generated SQL và actual plan trước khi thêm index.'),
  ...
  },
  {
    ...
    options: [
      option('a', 'Update kèm Version đã đọc; nếu không còn khớp thì trả conflict để user reload hoặc merge.', 'best', 'Đúng vì concurrency token phát hiện người khác đã sửa trong lúc form mở mà không giữ database lock quá lâu.'),
    ...
    explanation: { decision: 'Dùng concurrency token để phát hiện lost update rồi trả conflict rõ ràng.', mechanism: 'Update chỉ thành công khi Version còn giống lúc đọc; Version đã đổi nghĩa là có người save trước.', tradeOff: 'UI phải cho user reload hoặc merge; đổi lại không cần giữ lock suốt lúc form mở.', failureMode: 'Retry mù có thể ghi đè và làm mất thay đổi của người save trước.', evidence: 'Test hai update gần như cùng lúc và xác nhận API trả conflict cho request dùng Version cũ.' }, recall: ['Không muốn lost update: save kèm concurrency token.', 'Conflict là outcome cần xử lý, không chỉ là lỗi kỹ thuật.'], followUp: { changedConstraint: 'Đây là trừ tồn kho và quantity không được âm.', prompt: 'Cần thêm gì ở nơi ghi database?', expectedDirection: 'Dùng conditional update, constraint hoặc transaction ngắn để bảo vệ quantity; không chỉ dựa vào form.' }, tags: ['efcore', 'concurrency']
  },
  {
  ...
  },
  {
    ...
    options: [
      option('a', 'Lưu event ID hoặc Order ID trong cùng transaction với bản ghi giữ hàng để lần nhận lại không ghi thêm.', 'best', 'Đúng vì processed-event record — record đánh dấu event đã xử lý — và bản ghi giữ hàng cùng commit hoặc cùng rollback.'),
