...
function resolve(item: ReviewProgress) {
  if (item.kind === 'quiz') { const quiz = quizQuestions.find(question => `quiz:${question.id}` === item.id); return quiz ? { title: quiz.prompt, to: `/quiz/play?topic=${encodeURIComponent(quiz.topic)}`, kind: 'Quiz tình huống' } : null }
...
export default function ReviewPage() {
  const { dueItems, progress, remove } = useReviewProgress()
