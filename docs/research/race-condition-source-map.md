...
| [Microsoft Learn: `Barrier.SignalAndWait`](https://learn.microsoft.com/en-us/dotnet/api/system.threading.barrier.signalandwait) | Primary | Each participant signals and waits until the configured participant count reaches the barrier. | The controlled local reproduction; explicitly labeled teaching instrumentation. |
| [PostgreSQL: `UPDATE`](https://www.postgresql.org/docs/current/sql-update.html) | Primary | A conditional `WHERE` determines which rows are updated; `RETURNING` returns rows actually changed. | Scope and evidence wording for the database-boundary example. |
